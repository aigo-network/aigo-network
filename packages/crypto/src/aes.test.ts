...
describe('test AES', () => {
	...
	it('encrypt/decrypt with hardcode passcode, size-aligned data', async () => {
		const encryptedData = await aes.encryptByPasscode(alignedData, passcode1);
		const decryptedData = await aes.decryptByPasscode(encryptedData, passcode1);
	...
	it('encrypt/decrypt with hardcode passcode, size-unaligned data', async () => {
		const encryptedData = await aes.encryptByPasscode(unalignedData, passcode1);
		const decryptedData = await aes.decryptByPasscode(encryptedData, passcode1);
	...
	it('encrypt/decrypt with random passcode, size-aligned data', async () => {
		const encryptedData = await aes.encryptByPasscode(alignedData, passcode2);
		const decryptedData = await aes.decryptByPasscode(encryptedData, passcode2);
		expect(alignedData.toString()).toEqual(decryptedData.toString());
	});

	it('encrypt/decrypt with random passcode, size-unaligned data', async () => {
		const encryptedData = await aes.encryptByPasscode(unalignedData, passcode2);
		const decryptedData = await aes.decryptByPasscode(encryptedData, passcode2);
		expect(unalignedData.toString()).toEqual(decryptedData.toString());
	});
	...
	it('encrypt/decrypt with wrong passcode', async () => {
		try {
			...
			await aes.decryptByPasscode(encryptedData, passcode3);
		} catch (err) {
